...
- просмотр вкладки `Статистика` с количеством заявок по полю `Формат участия`;
- выгрузка всех заявок в Excel из вкладки `Статистика`;
- повторная постановка всех заявок с файлами публикаций в очередь автопроверки через скрытую служебную страницу.
...
- вкладка `Статистика` доступна только администратору по маршруту `/admin/statistics`, отображается в навигации после вкладки `Все заявки` и позволяет скачать Excel-файл со всеми заявками;
- служебная страница `/admin/maintenance` доступна только администратору, не показывается в навигации и рассчитана на прямой переход по URL;
- на странице обслуживания администратор может временно отключать или включать создание и удаление заявок.
...
- `Экспертное заключение` — необязательный `.docx` или `.pdf`, до 20 Мб;
- `Рецензия` — необязательный `.docx` или `.pdf`, до 20 Мб.
...
7. При необходимости администратор открывает `/admin/maintenance`, ставит все заявки с файлами публикаций на повторную автопроверку, и `checker` повторно заполняет `publication_validation`, включая поле замечаний автопроверки.
8. Администратор открывает `/all_applications`, выбирает заявку, меняет статус и при необходимости добавляет
   комментарий.
9. `web` создаёт задачу в коллекции `email_tasks`.
10. `mailer` забирает задачу из очереди и отправляет письмо через SMTP.
11. `mongo-backup` периодически делает резервную копию всей MongoDB.
...
- `GET /admin/statistics` — вкладка статистики с группировкой количества заявок по полю `Формат участия`;
- `GET /admin/statistics/export.xlsx` — выгрузка всех заявок в Excel;
- `GET /admin/maintenance` — скрытая страница обслуживания; не отображается в навигации и доступна только администратору при прямом переходе по URL;
- `POST /admin/maintenance` — сохраняет переключатели создания и удаления заявок;
- `POST /admin/maintenance/publication-validation-recheck` — повторно ставит в очередь автопроверки все заявки с загруженными файлами публикаций; результаты после обработки `checker` записываются в поле замечаний автопроверки `publication_validation.errors`;
