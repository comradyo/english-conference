...
- служебная страница `/admin/publication-validation-recheck` доступна только администратору, не показывается в навигации и рассчитана на прямой переход по URL.
...
7. При необходимости администратор открывает `/admin/publication-validation-recheck`, ставит все заявки с файлами публикаций на повторную автопроверку, и `checker` повторно заполняет `publication_validation`, включая поле замечаний автопроверки.
...
- `GET /admin/publication-validation-recheck` — скрытая страница повторной автопроверки файлов публикаций; не отображается в навигации и доступна только администратору при прямом переходе по URL;
- `POST /admin/publication-validation-recheck` — повторно ставит в очередь автопроверки все заявки с загруженными файлами публикаций; результаты после обработки `checker` записываются в поле замечаний автопроверки `publication_validation.errors`;
